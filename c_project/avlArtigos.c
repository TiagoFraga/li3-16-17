...
linkedList_node_t* linkedList_create_node(){
	...
	node->value = malloc(500*sizeof(char));
	node->next = NULL;
	return node;
}
...
char** querie9(avl_tree_t* tree, char* prefix){
	...
	while(nodo!=NULL){
		result[i] = malloc((strlen(nodo->value))*sizeof(char));
		strcpy(result[i],nodo->value);
		i++;
